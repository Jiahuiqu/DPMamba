...
class CrossScanTriton(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: torch.Tensor): # torch.Size([1, 96, 56, 56])
        B, C, H, W = x.shape # 1, 96, 56, 56
        B, C, H, W = int(B), int(C), int(H), int(W) # 1, 96, 56, 56
        BC, BH, BW = min(triton.next_power_of_2(C), 1), min(triton.next_power_of_2(H), 64), min(triton.next_power_of_2(W), 64) # 1  64  64
        NH, NW, NC = triton.cdiv(H, BH), triton.cdiv(W, BW), triton.cdiv(C, BC) # 96 1 1
        ctx.shape = (B, C, H, W) # (1, 96, 56, 56)
        ctx.triton_shape = (BC, BH, BW, NC, NH, NW) # (1, 64, 64, 96, 1, 1)
        x = x.contiguous()
