...
def build_optimizer(config, model, logger, **kwargs):
    ...
    if opt_lower == 'sgd':
        optimizer = optim.SGD(model.parameters(), momentum=config.TRAIN.OPTIMIZER.MOMENTUM, nesterov=True,
                              lr=config.TRAIN.LEARN_RATE, weight_decay=config.TRAIN.WEIGHT_DECAY)
...
def build_optimizer_fine_tuning(config, model, logger, **kwargs):
    """
    Build optimizer, set weight decay of normalization to 0 by default.
    """
    logger.info(f"==============> building optimizer {config.Fine_TUNE.OPTIMIZER.NAME}....................")

    opt_lower = config.Fine_TUNE.OPTIMIZER.NAME.lower()
    optimizer = None
    if opt_lower == 'sgd':
        optimizer = optim.SGD(model.parameters(), momentum=config.Fine_TUNE.OPTIMIZER.MOMENTUM, nesterov=True,
                              lr=config.Fine_TUNE.LEARN_RATE, weight_decay=config.Fine_TUNE.WEIGHT_DECAY)
    elif opt_lower == 'adamw':
        optimizer = optim.AdamW(model.parameters(), eps=config.Fine_TUNE.OPTIMIZER.EPS, betas=config.Fine_TUNE.OPTIMIZER.BETAS,
                                lr=config.Fine_TUNE.LEARN_RATE, weight_decay=config.Fine_TUNE.WEIGHT_DECAY)
    else:
        raise NotImplementedError
